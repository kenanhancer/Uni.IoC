...
namespace Uni.IoC
{
    public static class ProxyTypeFactory
    {
        ...
        {
            string name = String.Format("{0}_ProxyType", serviceType.FullName);

            Type proxyType = MemoryCache.Default.Get(name) as Type;
            if (proxyType == null)
            {
                ...
                AssemblyBuilder asmBuilder = AppDomain.CurrentDomain.DefineDynamicAssembly(new AssemblyName("DynamicAssembly"), AssemblyBuilderAccess.RunAndSave);
                ModuleBuilder moduleBuilder = asmBuilder.DefineDynamicModule("DynamicModule", typeName + ".dll");
                TypeBuilder typeBuilder = moduleBuilder.DefineType(typeName, TypeAttributes.Public);

                typeBuilder.AddInterfaceImplementation(IoCConstants.proxyInterfaceType);

                foreach (CustomAttributeData attr in serviceType.GetCustomAttributes(false))
                {
                ...
                staticConstructorIlGen.Emit(OpCodes.Call, UniConstants.GetTypeFromHandleMi);
                ...
                LocalBuilder objVariableLocalBuilder = staticConstructorIlGen.DeclareLocal(UniConstants.ObjectType);
                ...
                for (int i = 0; i < serviceMethods.Length; i++)
                {
                    ...
                    dictKey = String.Format("{0}_{1}",mi.Name, mi.GetHashCode());
                ...
                constructorSecondILGen.Emit(OpCodes.Call, UniConstants.ObjectCi);
                ...
                for (int i = 0; i < serviceMethods.Length; i++)
                {
                    ...
                    methodIlGen.Emit(OpCodes.Newarr, UniConstants.ObjectType);
                ...
                MemoryCache.Default.Add(name, proxyType, DateTimeOffset.Now.AddHours(2));
            }
        ...
        public static Dictionary<string, ProxyMethod> CreateMethodInvokers(Type serviceType)
        {
            ...
            foreach (MethodInfo method in serviceMethods)
            {
                ...
                proxyMethodsDict.Add(String.Format("{0}_{1}", method.Name, method.GetHashCode()), proxyMethod);
            }
